...
pub fn str_to_bigdecimal(val: &str, field: &str) -> Result<BigDecimal, ApiError> {
    BigDecimal::from_str(val).map_err(|_| {
        ApiError::bad_request()
            .code(ApiErrorCode::InvalidNumber)
            .message(format!("Invalid number for {}", field))
    })
}

pub fn str_opt_to_bigdecimal(
    val_opt: Option<String>,
    field: &str,
) -> Result<Option<BigDecimal>, ApiError> {
    if let Some(val) = val_opt {
        return BigDecimal::from_str(&val).map(|v| Some(v)).map_err(|_| {
            ApiError::bad_request()
                .code(ApiErrorCode::InvalidNumber)
                .message(format!("Invalid number for {}", field))
        });
